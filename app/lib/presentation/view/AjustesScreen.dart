...
class AjustesScreen extends StatefulWidget {
  ...
  user; // Trae 'nombres', 'apellidos', 'correo', 'id_usuario' del Login
...
class _AjustesScreenState extends State<AjustesScreen> {
  ...
  bool isFetching =
      true; // Controla la pantalla de carga inicial al traer los datos
  bool isEditing =
      false; // Controla si los campos están habilitados para edición
  ...
  Map<String, List<String>> datosColombiaGlobal = {};

  Future<void> cargarDepartamentosMunicipios() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var item in data) {
          String departamento = item['departamento'].toString().toUpperCase();

          List<String> municipios = List<String>.from(item['ciudades']);

          municipios.sort();

          temp[departamento] = municipios;
        }

        setState(() {
          datosColombiaGlobal = temp;
        });
      }
    } catch (e) {
  ...
  void initState() {
    Future<void> _inicializarPantalla() async {
      await cargarDepartamentosMunicipios();
      await _cargarDatosDesdeServidor();
    }
    ...
    nombresController = TextEditingController(
      text: (widget.user['nombres'] ?? '').toString(),
    );
    apellidosController = TextEditingController(
      text: (widget.user['apellidos'] ?? '').toString(),
    );
  ...
  Future<void> _cargarDatosDesdeServidor() async {
    try {
      final result = await _apiService.obtenerDatosAdicionales(
      ...
      if (result['success'] == true && result['existe'] == true) {
        ...
        setState(() {
          ...
          telefonoEmergenciaController.text =
              (datosBD['telefono_emergencia'] ?? '').toString();
          numeroDocumentoController.text = (datosBD['numero_documento'] ?? '')
              .toString();

          // Validar y asignar Departamento
          String? deptoBD = datosBD['departamento']?.toString().toUpperCase();

          if (deptoBD != null && datosColombiaGlobal.containsKey(deptoBD)) {
            selectedDepartamento = deptoBD;

            String? muniBD = datosBD['municipio']?.toString();

            if (muniBD != null &&
                datosColombiaGlobal[deptoBD]!.any(
                  (m) => m.toLowerCase() == muniBD.toLowerCase(),
                )) {
              selectedMunicipio = datosColombiaGlobal[deptoBD]!.firstWhere(
                (m) => m.toLowerCase() == muniBD.toLowerCase(),
              );
            }
          }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Error al conectar e intentar cargar datos: $e"),
  ...
  Future<void> _saveChanges() async {
    ...
    if (nombresController.text.trim().isEmpty ||
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "❌ Todos los campos son estrictamente obligatorios para poder guardar.",
          ),
          backgroundColor: Colors.red,
        ),
      );
    ...
    try {
      ...
      if (fechaActual.month < fechaNac.month ||
          (fechaActual.month == fechaNac.month &&
      ...
      if (edad < 16) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "❌ Restricción de edad: Debes tener al menos 16 años para registrarte.",
            ),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("❌ Formato de fecha de nacimiento incorrecto."),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    if (telefonoMovil.length != 10 || !telefonoMovil.startsWith('3')) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "❌ El Teléfono Móvil debe tener 10 dígitos y comenzar con 3.",
          ),
          backgroundColor: Colors.red,
        ),
      );
    ...
    if (telefonoEmergencia.length != 10 ||
        !telefonoEmergencia.startsWith('3')) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "❌ El Teléfono de Emergencia debe tener 10 dígitos y comenzar con 3.",
          ),
          backgroundColor: Colors.red,
        ),
      );
    ...
    if (telefonoMovil == telefonoEmergencia) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "⚠️ El número de teléfono móvil no puede ser el mismo que el de emergencia",
          ),
  ...
  Widget build(BuildContext context) {
    ...
    if (selectedDepartamento != null &&
        datosColombiaGlobal.containsKey(selectedDepartamento)) {
      listaMunicipiosActiva = List<String>.from(
        datosColombiaGlobal[selectedDepartamento]!,
      );
